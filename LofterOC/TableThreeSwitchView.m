...
@interface TableThreeSwitchView () 
...
@implementation TableThreeSwitchView
...
- (void)layoutSubviews {
    ...
    self.leftButton.frame = CGRectMake(0, 0, self.frame.size.width / 3, self.frame.size.height);
    self.midButton.frame = CGRectMake(self.frame.size.width / 3, 0, self.frame.size.width / 3, self.frame.size.height);
    self.rightButton.frame = CGRectMake(self.frame.size.width / 3 * 2, 0, self.frame.size.width / 3, self.frame.size.height);
    self.bottomLineView.frame = CGRectMake(0, self.frame.size.height - 2, self.frame.size.width / 3, 2);
}
...
- (void)buttonClick:(UIButton *)sender {
    ...
    [self.delegate tableThreeSwitchViewDelegate:self didSelectedButton:self.selectedButton forIndex:(self.selectedButton.tag - 1)];
}

- (void)bottomViewAnimationWithIndex:(CGFloat)index {
    [UIView animateWithDuration:0.2 animations:^{
        CGRect bottomFrame = self.bottomLineView.frame;
        bottomFrame.origin.x = index * (self.frame.size.width / 3);
